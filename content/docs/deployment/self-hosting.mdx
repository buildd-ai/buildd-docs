...
<Callout type="info">
Hourly is deliberate, not a minimum viable setting. The tick only needs to be as
fine as the finest schedule you actually run, and a finer tick wakes the database
on every call for nothing. If you add a schedule finer than hourly, make the tick
at least that fine or it will fire late — see
[Task Schedules](/docs/features/schedules).
</Callout>
