...
| Field | Description |
|-------|-------------|
| `model` | Which Claude model to use (`opus`, `sonnet`, `haiku`, or `inherit`) |
...
| `mcpServers` | MCP server configurations with `${VAR}` interpolation for secrets |
| `requiredEnvVars` | Maps env var names to secret labels for credential injection |
...
MCP server configs use `${VAR}` syntax — the actual secret values are stored encrypted in the [secrets system](/docs/concepts/secrets) and injected at claim time.
...
- **Origin tracking** — `manual` (user-created) or `promoted` (from team registry)
