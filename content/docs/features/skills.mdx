---
title: Skills
description: Hybrid skill registry — team-wide standards plus workspace-scoped bindings with local scanning and promotion
---

# Skills

Skills are reusable instruction packages that give agents domain-specific expertise. Each skill is a `SKILL.md` file — the [open standard](https://docs.anthropic.com/en/docs/agents-and-tools/agent-skills) adopted by Anthropic, OpenAI, and the broader agent ecosystem — stored in a hybrid registry and delivered to agents automatically when they claim tasks.
...
Buildd uses a **hybrid skill model** with two tiers:

- **Team skills** (`skills` table) — Canonical definitions shared across all workspaces in a team. Admin-managed, versioned by content hash. Think of these as your org's standard operating procedures.
- **Workspace skills** (`workspace_skills` table) — Per-project bindings tied to a specific workspace (git repo). Can be created manually, discovered via local scanning, or promoted from workspace to team.

```
[.claude/skills/ on disk] ──scan──▶ [workspace_skills]  ──promote──▶ [skills (team)]
                                     origin='scan'                     shared across team
                                     enabled=false
                                     (pending review)
```

When resolving skills for a task, Buildd checks **workspace skills first** (enabled only), then falls back to **team skills**. This lets workspaces override team defaults with project-specific variations.
...
## Promoting Skills to Team

Workspace skills that prove useful across multiple projects can be promoted to the team registry. Promoted skills become available to all workspaces in the team.

### From the Dashboard

1. Navigate to your workspace's **Skills** page
2. Find the skill you want to promote
3. Click **Promote to Team** — requires admin role
4. The skill is copied to the team registry and linked via `skillId`
5. The workspace skill's `origin` changes to `promoted`

### From the API

```bash
curl -X POST https://buildd.dev/api/workspaces/{workspace-id}/skills/{skill-id}/promote \
  -H "Authorization: Bearer bld_xxx"
```

After promotion, the team skill acts as the canonical version. Other workspaces can reference it by slug and get the team-level content. The originating workspace keeps its local copy linked to the team skill.
...
- **Promote** workspace skills to the team registry (admin only)

Each skill shows an origin badge indicating how it was created: `manual`, `scan`, or `promoted`.
...
### Workspace Skill Endpoints

| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET` | `/api/workspaces/{id}/skills` | List workspace skills. `?enabled=true` to filter. |
| `POST` | `/api/workspaces/{id}/skills` | Create/upsert a skill. Requires `name` and `content`. Upserts by `(workspaceId, slug)`. |
| `GET` | `/api/workspaces/{id}/skills/{skillId}` | Get a single workspace skill with full content. |
| `PATCH` | `/api/workspaces/{id}/skills/{skillId}` | Update skill fields (content, enabled, description, etc.). |
| `DELETE` | `/api/workspaces/{id}/skills/{skillId}` | Delete a workspace skill binding. |
| `POST` | `/api/workspaces/{id}/skills/sync` | Batch upsert from local scanner. Sets `origin: 'scan'`. |
| `POST` | `/api/workspaces/{id}/skills/{skillId}/promote` | Promote to team registry. Requires admin role. |

### Team Skill Endpoints

| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET` | `/api/skills` | List team-level skills. |
| `POST` | `/api/skills` | Register a team skill. Accepts optional `content` alongside `contentHash`. |

### Workspace Skill Object
...
The `skillId` field is `null` for workspace-only skills and links to the team `skills` table when promoted.
...
| `scan` | Discovered from local `.claude/skills/` directory via scanner |
| `promoted` | Promoted to team registry — linked via `skillId` |

### MCP Tools

| Tool | Access | Description |
|------|--------|-------------|
| `buildd_list_skills` | All users | List workspace + team skills (merged view) |
| `buildd_register_skill` | Admin | Register a skill to the workspace |
| `buildd_create_task` | All users | Accepts `skillSlugs` array parameter |
...
### Syncing Skills from Scanner (API)

```bash
curl -X POST https://buildd.dev/api/workspaces/{workspace-id}/skills/sync \
  -H "Authorization: Bearer bld_xxx" \
  -H "Content-Type: application/json" \
  -d '{
    "skills": [
...
Synced skills land with `origin: 'scan'` and `enabled: false` by default.
...
The server resolves each slug against workspace skills (enabled only) first, then team skills. Resolved skill references are stored in `task.context.skillRefs`. The legacy `skillRef: { slug }` format is also supported for backward compatibility.
