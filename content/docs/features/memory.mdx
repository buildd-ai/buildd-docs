...
There is a sixth type, `summary`, which **can no longer be created** — `learn` and the memory write API reject it. But it is not gone: if your team has been using buildd for a while, `summary` rows may well outnumber every other type combined. They are returned by `recall` and the search API like any other type, and `type: "summary"` remains a valid **read** filter.

<Callout type="warn">
  Don't mistake "can't be written" for "doesn't exist". If you filter reads to the five writable types, you may be excluding most of your history. Omit the `type` filter, or pass `summary` explicitly, when you want the full picture.
</Callout>
