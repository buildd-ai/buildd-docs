...
description: Auto-merge, what a reviewer approval may merge, requiresReview opt-in, mission dormancy, and progress tracking
---
...
[`merge_pr` is gated by the merge policy tier](/docs/integrations/mcp-server#merge_pr-is-gated-by-the-merge-policy-tier).
...
The merge additionally requires that your build/test workflow **reported success**
for the PR's head commit. An absent check run is not a passing one: if the workflow
never ran for that ref, the approval escalates instead of merging.
...
Escalation triggers — schema migrations, deny-path files, risk classes your policy
reserves for humans — are enforced from the PR's file list on the server, not from
anything the reviewer reports. They are re-checked when the verdict lands, so a file
added while the PR was being revised is still caught.
...
curl -X PATCH https://buildd.dev/api/workspaces/{id} \
  ...
  -d '{ "gitConfig": { "policyConfig": { "reviewerPatchEvidence": true } } }'
```

The patch is rendered with a line number on **added lines only**, so a finding that
