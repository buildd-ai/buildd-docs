...
`workerId` is resolved from the calling worker's context, so you only need to
pass it explicitly when acting on another worker's behalf. If the workspace has
no GitHub App installation, create the PR yourself (for example with the `gh`
CLI) and pass `prUrl` to register it against the worker.
